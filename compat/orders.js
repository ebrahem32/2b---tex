(function() {
  function createOrderDomain(deps) {
    ...
    function orderAccessoryConfig(order = {}) {
      ...
      if (lines.length) {
        return lines.map((line) => ({
          ...
          quantityManual: line.quantityManual !== void 0 ? line.quantityManual : ""
    ...
    function normalizeOrderForRuntime(order = {}) {
      ...
      safe.orderDate = safe.orderDate || (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
    ...
    function allocationAccessoryQuantity(order, allocation) {
      if (allocation.accessoryQuantityManual !== null && allocation.accessoryQuantityManual !== void 0 && allocation.accessoryQuantityManual !== "") return roundNumber(Number(allocation.accessoryQuantityManual || 0));
    ...
    function calculateOrder(order) {
      ...
      const singleWidthValue = order.widthMode !== "multiple" ? (_a = baseAllocations.find((item) => item.targetFinishedWidth)) == null ? void 0 : _a.targetFinishedWidth : null;
      const singleWeightValue = order.widthMode !== "multiple" ? (_b = baseAllocations.find((item) => item.targetFinishedWeight)) == null ? void 0 : _b.targetFinishedWeight : null;
      ...
      const configuredAccessoryLines = orderAccessoryConfig(order).map((line) => {
        const quantity = line.quantityManual !== "" && line.quantityManual !== null && line.quantityManual !== void 0 ? Number(line.quantityManual || 0) : Number(order.totalRawQuantity || 0) * Number(line.percent || 0) / 100;
