...
function renderPricings() {
  ...
  const buildGroupedRows = (sourcePricings, mode) => {
    ...
    const body = rows.map((pricing)=>{
      ...
      const financialCenterButton = linkedOrder ? `<button class="mini-btn gold" data-open-order-financial="${escapeHtml(linkedOrder.id)}">المركز المالي</button>` : '';
