...
function openManagementReport(type) {
  ...
  if (type === 'inventory') {
    ...
    list.forEach((order)=>order.allocations.forEach((allocation)=>{
      ...
      if (balance > 0) clothRows.push(`<tr><td>${escapeHtml(order.orderNumber)}</td><td>${escapeHtml(order.customer)}</td><td>${escapeHtml(order.fabricType || '-')}</td><td>${escapeHtml(allocation.color || '-')}</td><td>${escapeHtml(widthInfo.inch)}</td><td>${escapeHtml(widthInfo.width)}</td><td>${reportFmt(allocation.finishedReceived)}</td><td>${reportFmt(delivered)}</td><td><strong>${reportFmt(balance)}</strong></td></tr>`);
      if (order.accessoryLines.length) {
        order.accessoryLines.forEach((line)=>{
          ...
          if (accessoryBalance > 0) accessoryRows.push(`<tr><td>${escapeHtml(order.orderNumber)}</td><td>${escapeHtml(order.customer)}</td><td>${escapeHtml(allocation.color || '-')}</td><td>${escapeHtml(accessoryLineName(line, order))}</td><td>${reportFmt(received)}</td><td>${reportFmt(customerDelivered)}</td><td><strong>${reportFmt(accessoryBalance)}</strong></td></tr>`);
        });
      }
    }));
  ...
  if (type === 'delays') {
    const rows = list.filter((order)=>cleanOperationalStage(getOperationalStage(order))!=='مكتمل').map((order)=>{ const stageDate = stageStartDate(order); return { order, stage:cleanOperationalStage(getOperationalStage(order)), stageDate, days:daysSince(stageDate) }; }).sort((a,b)=>b.days-a.days).map(({order,stage,stageDate,days})=>`<tr><td>${escapeHtml(order.orderNumber)}</td><td>${escapeHtml(order.customer)}</td><td>${escapeHtml(order.fabricType || '-')}</td><td>${escapeHtml(order.dyehouse || '-')}</td><td>${escapeHtml(stage)}</td><td>${escapeHtml(stageDate || '-')}</td><td><strong>${days}</strong></td><td>${reportFmt(order.totalRawOrdered)}</td><td>${reportFmt(order.totalFinishedReceived)}</td><td>${reportFmt(order.totalDeliveredToCustomer)}</td></tr>`).join('');
  ...
  if (type === 'customer-account') {
    ...
    const rows = Object.entries(groups).sort((a,b)=>a[0].localeCompare(b[0],'ar')).map(([customer,data])=>`<tr><td>${escapeHtml(customer)}</td><td>${data.count}</td><td>${reportFmt(data.contract,2)}</td><td>${reportFmt(data.raw)}</td><td>${reportFmt(data.finished)}</td><td>${reportFmt(data.delivered)}</td><td>${reportFmt(data.balance)}</td><td>${data.open}</td></tr>`).join('');
  ...
  if (type === 'order-movement') {
    const rows = list.map((order)=>{ const dates = orderMovementDates(order); return `<tr><td>${escapeHtml(order.orderNumber)}</td><td>${escapeHtml(order.customer)}</td><td>${escapeHtml(order.fabricType || '-')}</td><td>${escapeHtml(dates.orderDate)}</td><td>${escapeHtml(dates.weavingDate)}</td><td>${escapeHtml(dates.dyehouseDate)}</td><td>${escapeHtml(dates.customerDate)}</td><td>${escapeHtml(cleanOperationalStage(getOperationalStage(order)))}</td></tr>`; }).join('');
  ...
  if (type === 'raw-returns') {
    const rows = getRawReturns().map((batch)=>{ const allocation = getAllocations().find((item)=>item.id===batch.allocationId); const sourceOrder = getOrders().find((item)=>item.id===batch.orderId || item.id===allocation?.orderId); const order = sourceOrder ? calculateOrder(sourceOrder) : { orderNumber:'-', customer:'-' }; return `<tr><td>${escapeHtml(batch.date || '-')}</td><td>${escapeHtml(order.orderNumber || '-')}</td><td>${escapeHtml(order.customer || '-')}</td><td>${escapeHtml(allocation?.color || '-')}</td><td>${escapeHtml(allocation?.dyehouse || '-')}</td><td>${reportFmt(batch.quantity)}</td><td>${escapeHtml(batch.noteNumber || '-')}</td><td>${escapeHtml(batch.notes || '-')}</td></tr>`; }).join('');
  ...
  if (type === 'accessories') {
    ...
    list.filter((order)=>order.accessoryLines.length).forEach((order)=>order.allocations.forEach((allocation)=>{
      order.accessoryLines.forEach((line)=>{
        ...
        rows.push(`<tr><td>${escapeHtml(order.orderNumber)}</td><td>${escapeHtml(order.customer)}</td><td>${escapeHtml(allocation.color || '-')}</td><td>${escapeHtml(accessoryLineName(line, order))}</td><td>${reportFmt(required)}</td><td>${reportFmt(sent)}</td><td>${reportFmt(received)}</td><td>${reportFmt(delivered)}</td><td>${reportFmt(received - delivered)}</td></tr>`);
      });
    }));
  ...
  list.forEach((order)=>{
    (order.allocations || []).forEach((allocation)=>{
      ...
      if (remaining > 0) {
        ...
        detailRows.push(`<tr><td>${escapeHtml(name)}</td><td>${escapeHtml(order.orderNumber || '-')}</td><td>${escapeHtml(order.customer || '-')}</td><td>${escapeHtml(order.fabricType || '-')}</td><td>${escapeHtml(allocation.color || '-')}</td><td>${reportFmt(allocation.plannedQuantity || 0)}</td><td>${reportFmt(sent)}</td><td>${reportFmt(received)}</td><td><strong>${reportFmt(remaining)}</strong></td><td>${sentDate || '-'}</td><td>${receivedDate || '-'}</td><td>${waitingDays}</td></tr>`);
      }
    });
  });
  const rows = Object.entries(groups).filter(([, data])=>reportNumber(data.remaining) > 0).sort((a,b)=>b[1].remaining - a[1].remaining).map(([name,data])=>`<tr><td>${escapeHtml(name)}</td><td>${data.activeOrders.size}</td><td>${data.activeColors}</td><td>${reportFmt(data.sent)}</td><td>${reportFmt(data.received)}</td><td><strong>${reportFmt(data.remaining)}</strong></td><td>${data.oldestDate || '-'}</td><td>${data.maxDays}</td></tr>`).join('');
