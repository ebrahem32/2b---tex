...
const APP_VERSION = 'v2026.08.18.03';
const APP_BUILD_TIME = '2026-08-18 13:48';
...
function approximateNamedColorHex(value) {
  const name = String(value || '').trim().toLocaleLowerCase('ar').replace(/[أإآ]/g, 'ا').replace(/ة/g, 'ه').replace(/ى/g, 'ي');
  const palette = [
    [/(اوف\s*وايت|سكري|عاجي)/, '#eee9dc'], [/(ابيض)/, '#fafafa'], [/(اسود|فاحم)/, '#171717'],
    [/(رصاصي|رمادي).*?(غامق|داكن)|(غامق|داكن).*?(رصاصي|رمادي)/, '#4b5055'], [/(رصاصي|رمادي).*?فاتح|فاتح.*?(رصاصي|رمادي)/, '#a9adb1'], [/(رصاصي|رمادي)/, '#777d82'],
    [/(بترولي)/, '#17666a'], [/(كحلي)/, '#172d46'], [/(سماوي)/, '#9dccd2'], [/(ازرق\s*ملكي)/, '#214b9a'], [/(ازرق)/, '#356b9d'],
    [/(اخضر).*?(غامق|داكن)|(غامق|داكن).*?(اخضر)/, '#214b35'], [/(اخضر)/, '#3e7754'], [/(زيتي)/, '#596334'],
    [/(نبيتي|عنابي)/, '#681f32'], [/(احمر)/, '#a92d35'], [/(روز|وردي|بينك)/, '#d78fa7'], [/(موف|بنفسجي)/, '#76527d'],
    [/(بني.*?(غامق|داكن)|شوكولات)/, '#5b3526'], [/(بني)/, '#79543e'], [/(بيج|كشمير|جملي)/, '#c6ad86'],
    [/(اصفر|مسترده)/, '#d5ab3c'], [/(برتقالي|اورنج)/, '#cf6f32'],
  ];
  return palette.find(([pattern])=>pattern.test(name))?.[1] || '#8b95a1';
}
function namedColorLabelHtml(value) {
  const label = String(value || '-').trim() || '-';
  return `<span class="named-color-label"><i class="named-color-swatch" style="background:${approximateNamedColorHex(label)}" title="معاينة تقريبية من اسم اللون"></i><span>${escapeHtml(label)}</span></span>`;
}
window.approximateNamedColorHex = approximateNamedColorHex;
...
const rawComponentsBox = document.createElement('div');
rawComponentsBox.id = 'rawComponentsBox';
rawComponentsBox.className = 'full-row raw-components-box';
rawComponentsBox.innerHTML = '<div class="subsection-head"><div><span>مكونات تشغيل الخام</span><p class="eyebrow">كل خام أو شانيه يدخل كسطر تشغيل مستقل حتى لو كانت المواصفات متشابهة.</p></div><button type="button" class="mini-btn" id="addRawComponentBtn">+ إضافة مكون</button></div><div class="raw-components-head"><span>المكون</span><span>المواصفة / النسبة</span><span>كمية العميل</span><span></span></div><div id="rawComponentsEditor"></div>';
...
function rawComponentRowHtml(component = {}) {
  return `<div class="raw-component-row">
    <input data-raw-component="name" value="${escapeHtml(component.name || '')}" placeholder="مثال: ممشط أو شانيه فاتح">
    <input data-raw-component="specification" value="${escapeHtml(component.specification || '')}" placeholder="مثال: 2% أو 8%">
    <input data-raw-component="quantity" type="number" step="0.01" min="0" value="${Number(component.quantity || 0) || ''}" placeholder="كمية العميل">
    <button type="button" class="mini-btn danger" data-remove-raw-component>حذف</button>
  </div>`;
}
function renderRawComponentsEditor(components = []) {
  if (!refs.rawComponentsEditor) return;
  refs.rawComponentsEditor.innerHTML = (Array.isArray(components) ? components : []).map(rawComponentRowHtml).join('');
}
function readRawComponentsEditor() {
  return [...(refs.rawComponentsEditor?.querySelectorAll('.raw-component-row') || [])].map((row)=>({
    name:String(row.querySelector('[data-raw-component="name"]')?.value || '').trim(),
    specification:String(row.querySelector('[data-raw-component="specification"]')?.value || '').trim(),
    quantity:Number(row.querySelector('[data-raw-component="quantity"]')?.value || 0),
...
const persistenceWriter = window.createPersistenceWriter({
  request: backendRequest,
  isAvailable: () => backendAvailable,
  markUnavailable: () => { backendAvailable = false; },
});
...
const orderToApi = (order, customerId = null) => ({
  ...
  order_type: order.orderType === 'manufacturing' ? 'manufacturing' : 'trade',
...
async function postBackend(path, payload) {
  return persistenceWriter.post(path, payload);
}
async function postBackendStrict(path, payload) {
  return persistenceWriter.post(path, payload, { strict:true });
}
async function putBackend(path, payload) {
  return persistenceWriter.put(path, payload);
}
async function deleteBackend(path) {
  return persistenceWriter.delete(path);
}
async function saveBackendSetting(key, value) {
  return persistenceWriter.saveSetting(key, value);
}
...
function openPantoneColorExperiment() {
  const dialog = document.createElement('dialog');
  dialog.className = 'transfer-choice-dialog pantone-test-dialog';
  dialog.innerHTML = `<form method="dialog" class="transfer-choice-card pantone-test-card" dir="rtl">
    <div class="subsection-head"><div><h3>تجربة لون البانتون</h3><p>ابحث برقم البانتون ثم التقط الدرجة من الشاشة. هذه معاينة تقريبية ولا تُحفظ في الطلب.</p></div><button type="button" class="mini-btn" data-close-pantone-test>إغلاق</button></div>
    <div class="pantone-test-fields"><label><span>رقم البانتون</span><input data-pantone-test-code placeholder="مثال: 19-4526 TPG"></label><label><span>اسم اللون</span><input data-pantone-test-name placeholder="مثال: Blue Coral"></label></div>
    <div class="pantone-test-preview"><span data-pantone-test-circle></span><div><strong data-pantone-test-label>معاينة اللون</strong><small data-pantone-test-hex>#326578</small></div></div>
    <div class="pantone-test-actions"><button type="button" class="mini-btn" data-search-pantone-test>بحث صور Google</button><button type="button" class="primary-btn" data-pick-pantone-test>التقاط اللون من الشاشة</button><label class="mini-btn pantone-manual-picker">اختيار يدوي<input type="color" data-pantone-test-color value="#326578"></label></div>
  </form>`;
  const codeInput = dialog.querySelector('[data-pantone-test-code]');
  const nameInput = dialog.querySelector('[data-pantone-test-name]');
  const circle = dialog.querySelector('[data-pantone-test-circle]');
  const label = dialog.querySelector('[data-pantone-test-label]');
  const hex = dialog.querySelector('[data-pantone-test-hex]');
  const colorInput = dialog.querySelector('[data-pantone-test-color]');
  const updatePreview = (color) => {
    const value = color || colorInput.value || '#326578';
    circle.style.backgroundColor = value;
    colorInput.value = value;
    hex.textContent = value.toUpperCase();
    label.textContent = [nameInput.value.trim(), codeInput.value.trim()].filter(Boolean).join(' — ') || 'معاينة اللون';
  };
  dialog.addEventListener('input', ()=>updatePreview());
  dialog.addEventListener('click', async (event) => {
    if (event.target.closest('[data-close-pantone-test]')) { dialog.close(); return; }
    if (event.target.closest('[data-search-pantone-test]')) {
      const query = [codeInput.value.trim(), nameInput.value.trim(), 'Pantone color chip'].filter(Boolean).join(' ');
      if (!query) { alert('اكتب رقم البانتون أولًا.'); return; }
      window.open(`https://www.google.com/search?tbm=isch&q=${encodeURIComponent(query)}`, '_blank', 'noopener');
      return;
    }
    if (!event.target.closest('[data-pick-pantone-test]')) return;
    if (!window.EyeDropper) { colorInput.click(); return; }
    try { const result = await new window.EyeDropper().open(); updatePreview(result.sRGBHex); } catch {}
  });
  dialog.addEventListener('close', ()=>dialog.remove(), { once:true });
  document.body.appendChild(dialog);
  updatePreview('#326578');
  dialog.showModal();
  codeInput.focus();
}
...
function pricingContractSourceForOrder(pricing, operationalOrder = null) {
  ...
  const syncAccessories = (lines = []) => lines.map((line)=>{
    const matching = orderAccessories.find((current)=>normalizeForCompare(current.type) === normalizeForCompare(line.type))
      || (orderAccessories.length === 1 && lines.length === 1 ? orderAccessories[0] : null);
    if (!matching) return line;
    const operationalQuantity = orderAccessoryQuantity(matching);
    const pricingQuantity = Number(line.quantity ?? line.quantityManual ?? 0);
    const syncedQuantity = operationalQuantity > 0 ? operationalQuantity : pricingQuantity;
    return {
      ...line,
      quantity:syncedQuantity,
      quantityManual:syncedQuantity,
      percent:Number(matching.percent || line.percent || 0),
    };
  });
...
function filteredOrders() {
  ...
  const orderType = refs.orderTypeFilter?.value || 'all';
  ...
  return allOrders().filter((order) => {
    ...
    return orderSearchText(order).includes(query) && statusMatch && (orderType === 'all' || order.orderType === orderType) && (customer === 'all' || order.customer === customer) && (dyehouse === 'all' || order.dyehouse === dyehouse) && (fabric === 'all' || order.fabricType === fabric);
  });
}
...
function accessoryDocumentSection(order, fmt, safe) {
  ...
  const rows = (lines.length ? lines : [{ type:'إكسسوار', percent:order?.accessoryPercent || 0, quantity:order?.accessoryRequired || 0 }])
    .map((line) => {
      const quantity = Number(line.quantity || line.quantityManual || 0);
      const percent = quantity > 0 && orderTotal > 0 ? roundNumber(quantity / orderTotal * 100) : Number(line.percent || 0);
      return `<tr><td>${safe(line.type || 'إكسسوار')}</td><td>${formatNumber(percent)}%</td><td>${fmt(quantity)}</td></tr>`;
    }).join('');
...
function allocationMovementLabel(order, allocation) {
  if (!allocation) return '-';
  return `${colorWithPantoneLabel(allocation)} / ${allocation.dyehouse || '-'} / ${allocationWidthLabel(order, allocation)}`;
}
function productionMovementLabel(order, allocation, batch) {
  if (!allocation) return '-';
  return `${colorWithPantoneLabel(allocation)} / ${batch?.dyehouse || allocation.dyehouse || '-'} / ${allocationWidthLabel(order, allocation)}`;
}
...
function allocationOptionLabel(order, allocation) {
  ...
  return `${ordinal ? `${ordinal} / ` : ''}${colorWithPantoneLabel(allocation)} / ${allocation.dyehouse || '-'}${allocationWidthSuffix(order, allocation)}${planned}`;
}
    ...
    if (table) {
      ...
      if (head && body) {
        ...
        body.innerHTML = scopedOrderDetailAllocationRows(order).map((allocation) => {
          ...
          return `<tr><td>${namedColorLabelHtml(allocation.color || '-')}</td><td>${plannedCell}</td><td>${escapeHtml(allocation.dyehouse || order.dyehouse || '-')}</td><td>${escapeHtml(allocationWidthLabel(order, allocation))}</td><td>${escapeHtml(allocation.targetFinishedWeight || '-')}</td><td>${sentCell}</td><td>${deliveredCell}</td><td><strong>${balanceCell}</strong></td><td>${wasteLabel}</td><td>${actions}</td></tr>`;
        }).join('');
      }
    }
  }
...
function fillOrderForm(order) {
  ...
  refs.orderType.value = order.orderType === 'manufacturing' ? 'manufacturing' : 'trade';
  refs.totalRawQuantity.value = order.totalRawQuantity || '';
  renderRawComponentsEditor(order?.operationNotes?.rawComponents || []);
  ...
  refs.orderPreparedWeight.value = order?.operationNotes?.preparedWeight || '';
  refs.orderPreparedWidth.value = order?.operationNotes?.preparedWidth || '';
...
async function addOrder(event) {
  ...
  const payloadOperationNotes = currentOrder?.operationNotes && typeof currentOrder.operationNotes === 'object' && !Array.isArray(currentOrder.operationNotes)
    ...
    : {};
  const rawComponents = readRawComponentsEditor();
  const rawComponentsTotal = roundNumber(rawComponents.reduce((total, line)=>total + Number(line.quantity || 0), 0));
  const customerQuantity = Number(refs.totalRawQuantity.value || 0);
  if (rawComponents.length && Math.abs(rawComponentsTotal - customerQuantity) > 0.01) {
    alert(`مجموع مكونات الخام (${formatNumber(rawComponentsTotal)}) يجب أن يساوي إجمالي طلب العميل (${formatNumber(customerQuantity)}).`);
    return;
  }
  payloadOperationNotes.rawComponents = rawComponents;
...
async function addBatch(event) {
  ...
  if (type === 'production') {
    ...
    if (!String(data.dyehouse || '').trim()) { alert('اختر المصبغة المستلم منها قبل تسجيل استلام المجهز.'); return; }
...
async function editAllocation(id) {
  ...
  if (!allocation) return;
  const order = orders.find((item)=>item.id===allocation.orderId);
  const colorValue = await window.TwoBTexInput.prompt('اكتب اسم اللون', allocation.color || '');
  if (colorValue === null) return;
  const cleanedColor = colorValue.trim();
  if (!cleanedColor) return;
  const pantoneValue = await window.TwoBTexInput.prompt('اكتب رقم البانتون (اختياري)', allocation.pantoneCode || '');
  if (pantoneValue === null) return;
  const cleanedPantone = pantoneValue.trim();
  ...
  if (hasDerby) {
    const currentRate = Number(allocation.plannedQuantity || 0) > 0 && allocation.accessoryQuantityManual !== null && allocation.accessoryQuantityManual !== undefined
      ? roundNumber(Number(allocation.accessoryQuantityManual || 0) / Number(allocation.plannedQuantity || 0) * 100)
      : '';
    const derbyPercentValue = await window.TwoBTexInput.prompt('نسبة الديربي لهذا اللون % (اتركها فارغة لاستخدام النسبة الموحدة)', currentRate);
    if (derbyPercentValue === null) return;
    const cleanedRate = String(derbyPercentValue).trim();
    accessoryQuantityManual = cleanedRate === '' ? null : roundNumber(Number(allocation.plannedQuantity || 0) * Number(cleanedRate || 0) / 100);
  }
  if (!(await ensureBackendForWrite())) return;
  const backendSaveRequired = true;
  const changedAllocations = new Set();

  const primaryUpdate = { ...allocation, color:cleanedColor, pantoneCode:cleanedPantone, accessoryQuantityManual };
  ...
  if (order?.widthMode !== 'multiple') {
    allocations.filter((item)=>item.orderId===allocation.orderId).forEach((item)=>{
      changedAllocations.add({
        ...
        pantoneCode: item.id === allocation.id ? cleanedPantone : item.pantoneCode,
        accessoryQuantityManual: item.id === allocation.id ? accessoryQuantityManual : item.accessoryQuantityManual,
        targetFinishedWidth,
...
function openBatchEditorDialog(type, batch) {
  ...
  const extraFields = type === 'raw'
    ...
    : type === 'production'
      ? `${field('المصبغة المستلم منها', 'dyehouse', batch.dyehouse || selectedAllocation.dyehouse || order.dyehouse || '', 'required')}`
    : type === 'transfer'
        ...
        : type === 'gluing'
          ...
          : type === 'customer'
            ...
            : type === 'finished'
              ...
              : '';
  const allocationField = type === 'production' && productionAllocationOptions
    ? `<label><span>اللون</span><select name="allocationId"><option value="">بدون تحديد</option>${productionAllocationOptions}</select></label>`
    : ['rawReturn','accessory','customer','finished'].includes(type) && allocationOptions
...
refs.openOrderFormBtn.onclick = () => { setOrderFormPricingConversionMode(false); pendingConvertedPricingId = null; pendingConvertedPricingItems = []; pendingConvertedOrderDrafts = []; editingOrderId = null; refs.orderForm.reset(); refs.orderNumber.value = nextPricingNumber(); refs.orderDate.value = new Date().toISOString().slice(0,10); syncAutoCodes(); renderWidthLinesEditor(); renderAccessoryLinesEditor(); renderRawComponentsEditor(); syncWidthModeUi(); resetGroupedOrderRows(); syncOrderTypeUi(); refs.orderDialog.showModal(); };
...
refs.addRawComponentBtn.onclick = () => refs.rawComponentsEditor.insertAdjacentHTML('beforeend', rawComponentRowHtml());
refs.rawComponentsEditor.onclick = (event) => { if (event.target.closest('[data-remove-raw-component]')) event.target.closest('.raw-component-row')?.remove(); };
...
refs.searchInput.oninput = refs.orderTypeFilter.oninput = refs.orderStatusFilter.oninput = refs.customerFilter.oninput = refs.dyehouseFilter.oninput = refs.fabricFilter.oninput = renderOrders;
