...
function renderDetails() {
  ...
  const gluingItems = listHtml(gluingBatches.filter((batch)=>batch.orderId===order.id), (batch)=>{ const allocation=order.allocations.find((item)=>item.id===batch.allocationId); const movement = String(batch.movement || 'sent'); const action = movement === 'received' ? 'استلام منتج ملزوق' : movement === 'customer' ? 'تسليم منتج ملزوق للعميل' : 'تسليم منتج مجهز للزق'; const name = batch.outputName || (allocation ? allocationColorLabel(order, allocation) : '-'); return batchItemHtml('gluing', batch, movementLine(action, batch.date, name, batch.quantity, batch.customerName || batch.partnerFabric || '-') + noteSuffix(batch)); });
  ...
  refs.orderDetailsPanel.querySelector('.batch-grid.compact')?.insertAdjacentHTML('beforeend', `<div class="batch-box"><h3>لزق خام</h3><form class="batch-form" data-form="gluing"><select name="movementKind" class="full"><option value="sent">تسليم منتج مجهز للزق</option><option value="received">استلام منتج ملزوق</option><option value="customer">تسليم المنتج الملزوق للعميل</option></select><select name="allocationId"><option value="">اختر اللون المصدر عند التسليم للزق</option>${order.allocations.map((allocation)=>`<option value="${allocation.id}">${allocationOptionLabel(order, allocation)}</option>`).join('')}</select><input name="date" type="date" required><input name="quantity" type="number" step="0.01" placeholder="الكمية" required><input name="partnerFabric" placeholder="الخامة الثانية / المصدر الآخر"><input name="outputName" placeholder="اسم المنتج بعد اللزق"><input name="customerName" placeholder="العميل"><input name="noteNumber" placeholder="رقم إذن"><input class="full" name="notes" placeholder="ملاحظات"><button class="mini-btn full">إضافة حركة لزق</button></form><div class="batch-list">${gluingItems}</div></div>`);
...
async function addBatch(event) {
  ...
  if (type === 'gluing') {
    data.movement = ['received', 'customer'].includes(data.movementKind) ? data.movementKind : 'sent';
    if (data.movement === 'sent') {
      if (!data.allocationId) { alert('اختر اللون المصدر الذي سيتم تسليمه للزق.'); return; }
      ...
      const available = Math.max(Number(allocation.finishedReceived || 0) - delivered - sentToGluing, 0);
