...
const APP_VERSION = 'v2026.06.07.20';
const APP_BUILD_TIME = '2026-06-07 20:09';
...
function consolidateOrderDetailView(order) {
  ...
  if (colorSection) {
    ...
    if (table) {
      const hasAccessories = order.accessoryLines.length > 0;
...
function openManagementReport(type) {
  ...
  if (type === 'dyehouse-performance') { if (refs.documentDialog.open) refs.documentDialog.close(); return openDyehouseBalancesReport(); }
...
function openDyehouseBalancesReport() {
  ...
  const labels = {
    ...
    dyehouse:'\u0627\u0644\u0645\u0635\u0628\u063a\u0629', activeOrders:'\u0623\u0648\u0627\u0645\u0631 \u0645\u0641\u062a\u0648\u062d\u0629 \u062f\u0627\u062e\u0644\u0647\u0627', activeColors:'\u0623\u0644\u0648\u0627\u0646 \u062f\u0627\u062e\u0644\u0647\u0627', sent:'\u0645\u0631\u0633\u0644 \u0644\u0644\u0645\u0635\u0628\u063a\u0629', received:'\u0645\u0633\u062a\u0644\u0645 \u0645\u0646 \u0627\u0644\u0645\u0635\u0628\u063a\u0629', remaining:'\u0627\u0644\u0631\u0635\u064a\u062f \u0627\u0644\u0641\u0639\u0644\u064a \u062f\u0627\u062e\u0644 \u0627\u0644\u0645\u0635\u0628\u063a\u0629', oldest:'\u0623\u0642\u062f\u0645 \u0625\u0631\u0633\u0627\u0644', order:'\u0631\u0642\u0645 \u0627\u0644\u0637\u0644\u0628', customer:'\u0627\u0644\u0639\u0645\u064a\u0644', item:'\u0627\u0644\u0635\u0646\u0641', color:'\u0627\u0644\u0644\u0648\u0646', sentDate:'\u062a\u0627\u0631\u064a\u062e \u0627\u0644\u0625\u0631\u0633\u0627\u0644', lastReceived:'\u0622\u062e\u0631 \u0627\u0633\u062a\u0644\u0627\u0645',
  ...
  list.forEach((order)=>{
    (order.allocations || []).forEach((allocation)=>{
      ...
      const sentDate = firstMovementDate(rawBatches, allocation.id, order.id);
      const receivedDate = firstMovementDate(productionBatches, allocation.id);
      groups[name] = groups[name] || { activeOrders:new Set(), activeColors:0, sent:0, received:0, remaining:0, oldestDate:'' };
      ...
      if (remaining > 0) {
        ...
        detailRows.push(`<tr><td>${name}</td><td>${order.orderNumber || '-'}</td><td>${order.customer || '-'}</td><td>${order.fabricType || '-'}</td><td>${allocation.color || '-'}</td><td>${reportFmt(sent)}</td><td>${reportFmt(received)}</td><td><strong>${reportFmt(remaining)}</strong></td><td>${sentDate || '-'}</td><td>${receivedDate || '-'}</td></tr>`);
      }
    });
  });
  const rows = Object.entries(groups).filter(([, data])=>reportNumber(data.remaining) > 0).sort((a,b)=>b[1].remaining - a[1].remaining).map(([name,data])=>`<tr><td>${name}</td><td>${data.activeOrders.size}</td><td>${data.activeColors}</td><td>${reportFmt(data.sent)}</td><td>${reportFmt(data.received)}</td><td><strong>${reportFmt(data.remaining)}</strong></td><td>${data.oldestDate || '-'}</td></tr>`).join('');
  const summaryHead = `<tr><th>${labels.dyehouse}</th><th>${labels.activeOrders}</th><th>${labels.activeColors}</th><th>${labels.sent}</th><th>${labels.received}</th><th>${labels.remaining}</th><th>${labels.oldest}</th></tr>`;
  const detailsHead = `<tr><th>${labels.dyehouse}</th><th>${labels.order}</th><th>${labels.customer}</th><th>${labels.item}</th><th>${labels.color}</th><th>${labels.sent}</th><th>${labels.received}</th><th>${labels.remaining}</th><th>${labels.sentDate}</th><th>${labels.lastReceived}</th></tr>`;
  const html = `<section class="report-section"><h3>${labels.summary}</h3><table class="follow-table"><thead>${summaryHead}</thead><tbody>${rows || `<tr><td colspan="7">${labels.emptySummary}</td></tr>`}</tbody></table></section><section class="report-section"><h3>${labels.details}</h3><table class="follow-table"><thead>${detailsHead}</thead><tbody>${detailRows.join('') || `<tr><td colspan="10">${labels.emptyDetails}</td></tr>`}</tbody></table></section>`;
