...
async function existingUniqueBusinessRow(table, data) {
  if (!data) return null;
  if (table === 'orders' && data.order_number && data.customer_id) {
    return get('SELECT id FROM orders WHERE order_number = ? AND customer_id = ? LIMIT 1', [data.order_number, data.customer_id]);
  }
  if (table === 'pricings' && data.pricing_number && data.customer_id) {
    return get('SELECT id FROM pricings WHERE pricing_number = ? AND customer_id = ? LIMIT 1', [data.pricing_number, data.customer_id]);
  }
  return null;
}
